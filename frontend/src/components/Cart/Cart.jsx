...
Logic Summary (Short & Crisp)

The Cart component displays the user’s shopping cart using data from Redux.

It fetches cart items and saved-for-later items from the global store using useSelector.

If the cart is empty, an EmptyCart component is shown.

If items exist, each product is rendered using the CartItem component.

A “Place Order” button:

Is disabled when the cart is empty

Redirects the user to login → shipping when clicked.

A separate section displays Saved For Later items.

A PriceSidebar calculates and shows the total price based on cart items.

The layout is responsive, adjusting for mobile and desktop.

MetaData sets the page title for SEO.
