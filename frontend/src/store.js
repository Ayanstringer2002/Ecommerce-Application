...
which acts as the single source of truth for the entire application state.
...
This file configures the Redux store for the entire application.

It combines multiple reducers into a single root reducer.

Redux Thunk is used to handle asynchronous operations like API calls.

Cart, wishlist, save-for-later, and shipping data are persisted using localStorage.

Redux DevTools are enabled for easier debugging.

The store acts as the centralized state management system for users, products, orders, and cart features.

⭐ One-Line Interview Summary

This Redux store setup centralizes application state, supports async actions via Redux Thunk, persists key data using localStorage, and enables scalable state management with modular reducers.
